...
function showCategory(category) {
    switch (category) {
        case "phones":
            makeResultingSet("phones")
            break;
        case "tablets":
            makeResultingSet("tablets");
            break;
        case "tv":
            makeResultingSet("tv");
            break;
...
/**
 * @param {string} category название категории товаров phones, tablets или tv.
 * 
 * */
function makeResultingSet(category) {
    let markupElem = '';
    products[category].forEach(function (product) {
        markupElem += getProductMarkup(product);
    });
    productsElem.insertAdjacentHTML("afterbegin", markupElem);
}
